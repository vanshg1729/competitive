...
struct segtree{
	...
	const int no_oper = 0;
	const int ini_val = 0;
	const int no_val = pow(2, 31) -1;

	int comb_op(int a, int b){
		return a & b;
	}

	int val_op(int a, int b){
		return a | b;
	}
	...
	int get(int k, int l, int r, int st, int en){
		...
		int right = get(2*k, l, m, st, min(en,m));
		int left = get(2*k + 1, m+1, r, max(st, m+1), en);
